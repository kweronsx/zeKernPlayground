...
int VectorAdd(sycl::queue &q1, sycl::queue &q2, sycl::queue &q3,
  ...
  int *sum_buf = (int *)malloc_device(3 * iter * 256 * sizeof(int), q1);
  size_t num_groups = 1;
  size_t wg_size = 256;
  ...
  for (int i = 0; i < iter; i++) {
    ...
    q1.submit([&](auto &h) {
      ...
      //        auto sum_acc = sum_buf[3 *
      //        i]->get_access<sycl::access::mode::write>(h);
      auto sum_acc = &sum_buf[3 * i * 256];
    ...
    q2.submit([&](auto &h) {
      ...
      //        auto sum_acc =
      //            sum_buf[3 * i +
      //            1]->get_access<sycl::access::mode::write>(h);
      auto sum_acc = &sum_buf[(3 * i + 1) * 256];
    ...
    q3.submit([&](auto &h) {
      ...
      //        auto sum_acc =
      //            sum_buf[3 * i +
      //            2]->get_access<sycl::access::mode::write>(h);
      auto sum_acc = &sum_buf[(3 * i + 2) * 256];
